...
import { computeLoanCost, type LoanTerms } from '../lib/finance';
...
interface LoanOption extends LoanTerms {
  name: string;
}
...
export default function LoanReceipt() {
  const [instant, bank] = LOAN_OPTIONS.map((option) => ({
    ...option,
    cost: computeLoanCost(option),
  }));
  const instantShareOfPrincipal = Math.round((instant.cost.totalCost / instant.principal) * 1000) / 10;
  const cheaperFactor = (instant.cost.totalCost / bank.cost.totalCost).toFixed(1);
    ...
    <section className="bg-[var(--paper)] py-16 px-4">
      <div className="max-w-6xl mx-auto">
        <SectionHeader
          ...
          description="Not &quot;45% effective APR&quot; — that means nothing. &quot;₹1,450 out of your account&quot; — that means everything."
        />

        {/* Three receipts side by side */}
        <div className="grid md:grid-cols-3 gap-6">
          {LOAN_OPTIONS.map((loan, index) => {
            const cost = computeLoanCost(loan);
            const isExpensive = cost.effectiveAnnualRate > 30;
            const isModerate = cost.effectiveAnnualRate >= 15 && cost.effectiveAnnualRate <= 30;
                  ...
                  <div className="receipt-line">
                    <div className="flex justify-between items-baseline">
                      <span className="text-[var(--ink-light)]">
                        Interest ({loan.annualInterestRate}% p.a.)
                      </span>
                      <span
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-[var(--ink)] leading-relaxed mb-4">
                A ₹{instant.principal.toLocaleString('en-IN')} instant loan at {instant.annualInterestRate}% APR costs you{' '}
                <strong>₹{instant.cost.totalCost.toLocaleString('en-IN')} extra</strong> in just{' '}
                {instant.tenureMonths} months. That's <strong>{instantShareOfPrincipal}% of your principal</strong>{' '}
                gone in fees and interest.
              </p>
              <p className="text-[var(--ink)] leading-relaxed">
                The bank personal loan at {bank.annualInterestRate}% APR costs only{' '}
                <strong>₹{bank.cost.totalCost.toLocaleString('en-IN')} extra</strong> over {bank.tenureMonths} months —
                {cheaperFactor}× cheaper for the same amount, even though it runs longer.
              </p>
